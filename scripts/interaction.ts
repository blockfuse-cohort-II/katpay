...
const main = async () => {
    ...
    //  // --- Function 1: getAmountsOut ---
    //  console.log("\n--- Calculating Expected Output (getAmountsOut) ---");
    //  const amountIn = ethers.parseUnits("1000", 6); // 1000 USDC
    //  const path = [USDCAddress, DAIAddress]; // USDC -> DAI
    //  const amountsOut = await uniswapContract.getAmountsOut(amountIn, path);
    //  const expectedDAIOutput = amountsOut[1];
    //  console.log(
    //      `Expected DAI Output for 1000 USDC: ${ethers.formatUnits(
    //          expectedDAIOutput,
    //          18
    //      )}`
    //  );

    //  // --- Function 2: getAmountsIn ---
    //  console.log("\n--- Calculating Required Input (getAmountsIn) ---");
    //  const amountOut = ethers.parseUnits("1000", 18); // 1000 DAI
    //  const amountsIn = await uniswapContract.getAmountsIn(amountOut, path);
    //  const requiredUSDCInput = amountsIn[0];
    //  console.log(
    //      `Required USDC Input for 1000 DAI: ${ethers.formatUnits(
    //          requiredUSDCInput,
    //          6
    //      )}`
    //  );
    ...
    //  console.log(
    //   "-------------------------- Swapping USDC for Exact DAI -------------"
    // );

    // const daiAmountOut = ethers.parseUnits("950", 18);
    // const usdcAmountInMax2 = ethers.parseUnits("1100", 6);
    ...
    // console.log(
    //   "-------------------------- USDC to Exact DAI Swap Completed -------------"
    // );
